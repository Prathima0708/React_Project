...
import "../NumberGuess/NumberGuessing.css";
...
function NumberGuessing() {
  ...
  const HandlerValueChange = (
    e //to know the value entered in the input field
  ) => {
  ...
  const SubmitHandler = () => {
    if (Number(RandomNumber) === Number(UserGuess)) {
      //comparing the value of random number with user guess
  ...
  return (
    <div className="number">
      ...
      {Disabled && <button onClick={RestartAgain}>Start Again</button>}

      <div className="msg">
        <p>Message : {Msg}</p>
        <p>Low or High : {LowHighMsg}</p>
        <p>Total rounds played by user : {UserCount}</p>{" "}
        {/*displaying total rounds played by user*/}
        <p>Random Number : {randNum}</p>
        <p>
          Your guesses :
          {UserAllGuessessVal.map((item, index) => {
            //iterating over the array to display user guesses.
            return (
              <span key={index}>
                {" "}
                {item}, {}
